...
export const Modal = ({ albums, visible, onSave, onClose }: P): JSX.Element => {
  ...
  return (
    <div className={classnames(styles.container, visible ? Display.visible : Display.hidden)}>
      {visible && (
            ...
            {({ isValid, setFieldValue, values }) => (
              <Form>
                <p className={styles.header}>
                  <span
                    className={values.action === ModalState.create ? "" : styles.toggled}
                    onClick={() => setFieldValue("action", ModalState.create)}
                  >
                    {_("messages.create-new-album")}
                  </span>
                  <span
                    onClick={() => setFieldValue("action", ModalState.add)}
                    className={values.action === ModalState.add ? "" : styles.toggled}
                  >
                ...
                {values.action === ModalState.create && (
                  <Field
                    ...
                    type="text"
                    placeholder={_("messages.enter-title-here")}
                    name="title"
                  />
                )}
                        ...
                        <ul>
                          {albums?.map((album, key) => (
                            <ListItem
                              ...
                              key={key}
                              item={album}
                              onSave={(index, value) => arrayHelpers.insert(index, value)}
                ...
                <Button type="reset">{_("messages.cancel")}</Button>
